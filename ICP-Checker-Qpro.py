...
def query_base(i):
    try:
        info = i.replace(" ", "").replace("https://www.", "").replace("http://www.",
                                                                                                        "").replace(
            "http://", "")
...
def get_beian_info(info_data, p_uuid, token, sign):
    ...
    try:
        ...
        if not beian_info["success"]:
            tqdm.write(f'请求错误: CODE {beian_info["code"]} MSG {beian_info["msg"]}')
            return domain_list
...
def data_saver(domain_list):
    ...
    try:
        ...
        tqdm.write("** 备案信息登记表格已打开，无法写入文件。如需写入，请关闭文件后重新执行！ **\n")
        return -1
    tqdm.write(f"查询结果保存在：{file_path}\n")
    return 'OK'
...
def getSourceFile(fileName):
    with open(fileName, 'r+') as f1:
        SourceList = [];
        [SourceList.append(i.strip()) for i in f1.readlines() if i.strip() not in SourceList]
        return SourceList


@click.command(context_settings=dict(help_option_names=['-h', '--help']))
@click.option('--file', '-f', help='一行一个的目标文件, 输入当前目录的文件名或文件绝对路径')
@click.option('--string', '-s', help='单个目标，支持域名、公司全称、备案号')
def main(file, string):
    tqdm.write("源项目版本：V2.1.7 可用测试：2023-9-5\n")
    tqdm.write("源项目地址：https://github.com/wongzeon/ICP-Checker\n")
    tqdm.write("当前项目：ICP-Checker-Qpro")
    cookie = get_cookies()
    ...
    # 判断及处理命令行输入
    if file:
        SourceList = getSourceFile(file)
    elif string:
        SourceList = []
        SourceList.append(string)
    else:
        raise ValueError("未输入目标")
    
    # 初始化进度条
    testnum = len(SourceList)
    # testtrange = trange(testnum,bar_format='{percentage:3.0f}%')
    testtrange = trange(testnum,desc='查询进度： ', ncols=50)
    ...
    with open("查询失败.txt", 'w+') as t:
        for i in findErrorDomain:
            t.write(i)
